...
import {DataContext, SheetContext} from '../Screen/VideoPlayer';
  ...
  const getCommnent = async () => {
    try {
      const response = await fetch(
        'https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet&maxResults=20&order=relevance&videoId=' +
          id +
          '&key=' +
          APIKEY,
      );
  ...
  if (!loading)
    return (
      <BottomSheetModalProvider>
        <TouchableOpacity onPress={value.handlePresentModalPress}>
          <View>
