...
function ConvertTime(Publictime){
    ...
    const [pYear, pMonth, pDay] = Pdate.split("-").map( d => { return parseInt(d) });
    const [pHour, pMinute, pSecond] = Ptime.substring(0,8).split(":").map( d => { return parseInt(d) });
...
function parseDuration(duration) {
  ...
  h = d? d*24 + h: h;
  return (  h?h.toString()+":" : "" )+ (h? (m?String("0" + m).slice(-2)+":" :"00:") : (m?m.toString() +":" :"0:")) + (s? String("0" + s).slice(-2) : "00");
          ...
          ({item}) => (
            ...
            <View style={styles.videoItem}>
              <View style={styles.videoThumb}>
                ...
                />
                <Text style={{color: "#fff", backgroundColor:"#000",position:"absolute", fontWeight:"bold", fontFamily:"ArialMT", fontSize:13, right:15, bottom:10 }}> {parseDuration(item.contentDetails.duration)}</Text>
              </View>
              <View  style={styles.describe}>
                <Image style={styles.channelImg} source={{uri: 'https://yt3.ggpht.com/9v791aXgd4UOgHOVAmwkoq77QplNF_NvZRS7O6hKmMtRT15W6iWXSXfDNihTkQOqIaeU-joNmg=s48-c-k-c0x00ffffff-no-rj'}}>
                </Image>
               <View style={{width:ScreenWidth*0.8}}>
                <Text style={{fontWeight:'500', }}>{item.snippet.title}</Text>
                  <View style={{flexDirection : "row", marginTop:5, }}>
                    <Text style={{color:"#484848", fontSize:12}}> {item.snippet.channelTitle} · {ConvertView(item.statistics.viewCount)} · {ConvertTime(item.snippet.publishedAt)}
                    </Text>
                  </View>
               </View>
              </View>
